<template>
    <GeneralCardComponent flat>
        <v-data-table dense :items="itemsTable(sample.name,type,items.description)" calculate-widths :headers="headers"
            hide-default-footer>
                ...
                <tr v-for="(item,index) in items" :key="`it`+index">
                    <td v-for="column in headers" class="text-center" :key="`cc`+column.id">
                        ...
                        <div v-if="column.value == 'actions'">
                            <v-btn small class="mt-3" color="red" @click="deleteItem(index)">
            ...
            <template v-slot:foot>
                <tr v-show="!readonly">
                    <td class="pa-2" v-for="column in items.headerTable" :key="`c${column.id}`">
                        <FormsFieldsTextComponent v-model="value[column.value]" placeholder="N/A"
                            v-if="!column.ecuation_field">
...
<script>
    ...
    export default {
        ...
        created() {
            this.propItems.headerTable = this.items.headerTable.map(item => {
                return {
                    ...item,
                    value: item.text.replace(" ", "_"),
                }
