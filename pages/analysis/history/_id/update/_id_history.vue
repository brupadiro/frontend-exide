<template>
    <v-container fluid>
        <generalCardComponent>
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn class="font-weight-bold black--text rounded-lg" color="yellow" @click="updateHistory()"> Update
                    history</v-btn>
            </v-card-actions>
        </generalCardComponent>
...
<script>
    ...
    export default {
        ...
        methods: {
            getHistory() {
                this.$axios.get('/histories/' + this.$route.params.id_history).then((response) => {
                    this.history = response.data.data
                })
            },
            updateHistory() {
                if(!this.$refs.form.validate())return
                this.$axios.put('/histories', {
                    data: this.history
                }).then(() => {
