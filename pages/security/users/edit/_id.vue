        ...
        <v-row>
            <v-col class="col-12">
                <GeneralCardComponent>
                    ...
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn class="font-weight-bold black--text rounded-lg" color="yellow" @click="updateUser()">
                            Update user</v-btn>
                    </v-card-actions>
                </GeneralCardComponent>
            </v-col>
    ...
    export default {
        ...
        methods: {
            getUser() {
                ...
                this.$axios.get(`/users/${id}/?populate=*` ).then((response) => {
                    this.user = response.data
                })
            },
            updateUser() {
                const id = this.$route.params.id
                this.$axios.put(`/users/${id}`,this.user).then(() => {
